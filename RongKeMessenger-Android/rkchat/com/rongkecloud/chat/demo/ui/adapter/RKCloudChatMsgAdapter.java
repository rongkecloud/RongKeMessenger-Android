...
public class RKCloudChatMsgAdapter extends BaseAdapter
{
	...
	public int getItemViewType(int position)
	{
		...
		{
			if (msgObj instanceof TipMessage)
			{
				type = ITEM_STYLE_TIP;
			}
			...
			if (MSG_DIRECTION.SEND == msgObj.getDirection())
			{
				...
				switch (msgObj.getStatus())
				{
					case SEND_SENDING:
						...
						itemBuffer.sendBtn.setImageResource(R.anim.rkcloud_chat_msg_sending);
	...
	private void showAudioMsg(View convertView, final AudioMessage msgObj)
	{
		...
		if (MSG_DIRECTION.SEND == msgObj.getDirection())
		{
			playImg.setImageResource(R.anim.rkcloud_chat_msg_audio_send_playing);
		}
		else
		{
			playImg.setImageResource(R.anim.rkcloud_chat_msg_audio_received_playing);
		}
