...
public class RKCloudChatPlayAudioMsgTools {
	...
	public Uri getNotificationUri() {
		...
		String notifiStr = RKCloudChatConfigManager.getInstance(mContext).getString(RKCloudChatConfigManager.NEWMSG_NOTICE_SOUND_URI);
		if (TextUtils.isEmpty(notifiStr)) {
			notifiToneUri = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION);
		} else {
			String path = "android.resource://" + mContext.getPackageName() + "/" + R.raw.rkcloud_chat_sound_custom;
			notifiToneUri = Uri.parse(path);
